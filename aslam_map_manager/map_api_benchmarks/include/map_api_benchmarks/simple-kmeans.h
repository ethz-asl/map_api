...
#ifndef MAP_API_BENCHMARKS_SIMPLE_KMEANS_H_
#define MAP_API_BENCHMARKS_SIMPLE_KMEANS_H_
...
class SimpleKmeans {
 public:
  ...
  typedef std::function<
      void(const std::vector<Feature*>&, size_t, Distance, int random_seed,
...
struct InitKMeansPlusPlus {
  ...
  template <class Distance, class FeatureAllocator>
  void operator()(const std::vector<Feature*>& descriptors, size_t k,
                  Distance distance, int random_seed,
                  std::vector<Feature, FeatureAllocator>* centers) {
    CHECK_NOTNULL(centers);
    std::mt19937 generator(random_seed);
    centers->resize(k, zero_);
    std::vector<double> minimum_distances(descriptors.size(),
    ...
    struct ParallelDistance {
      ParallelDistance(Distance _distance, const Feature& _feature,
    ...
    std::function<void(const Distance & distance, const Feature&,
                       const std::vector<Feature*>&,
                       std::vector<double>* distances)> parallel_distance =
      ...
      ParallelDistance distance_functor(distance, feature, features, distances);
      helpers::ParallelProcess<16>(features.size(), distance_functor);
    };
    ...
    std::vector<double> distances;
    parallel_distance(distance, (*centers)[num_centers - 1], descriptors,
                      &distances);
    ...
    while (num_centers < k) {
      ...
      double distance_sum = std::accumulate(minimum_distances.begin(),
                                            minimum_distances.end(),
                                            0.0);

      if (distance_sum > 0) {
        double random_cutoff;
        do {
          random_cutoff = (static_cast<double>(generator()) / RAND_MAX)
              * distance_sum;
        ...
        if (minimum_distance_iterator == minimum_distances.end()) {
          ...
          descriptor_idx = minimum_distance_iterator
              - minimum_distances.begin();
        }
