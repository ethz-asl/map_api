...
template <>
class FieldTest<dmap_common::Id> : public ::testing::Test {
 protected:
  dmap_common::Id sample_data_1() {
    dmap_common::Id id;
    id.fromHexString("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
    return id;
  }
  dmap_common::Id sample_data_2() {
    dmap_common::Id id;
...
class FieldTestWithoutInit
 ...
 protected:
  ...
  dmap_common::Id fillRevision(const typename TableDataType::DataType& value) {
    getTemplate();
    dmap_common::Id inserted;
    generateId(&inserted);
  ...
  dmap_common::Id fillRevision() { return fillRevision(this->sample_data_1()); }

  bool insertRevision() {
    return this->table_->insert(LogicalTime::sample(), query_);
  }

  void getRevision(const dmap_common::Id& id) {
...
#define ALL_DATA_TYPES(table_type)                                             \
  TableDataTypes<table_type, testBlob>,                                        \
      ...
      TableDataTypes<table_type, dmap_common::Id>,                                  \
...
TYPED_TEST(FieldTestWithInit, CreateRead) {
  dmap_common::Id inserted = this->fillRevision();
...
TYPED_TEST(CruMapIntTestWithInit, HistoryAtTime) {
  ...
  dmap_common::Id id = this->fillRevision(kFirst);
