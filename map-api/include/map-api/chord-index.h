...
class ChordIndex {
 public:
  ...
  bool handleFetchResponsibilities(const PeerId& requester,
 ...
 private:
  ...
  // should be known
  virtual bool fetchResponsibilitiesRpc(const PeerId& to,
                                        DataMap* responsibilities) = 0;
  virtual bool pushResponsibilitiesRpc(const PeerId& to,
                                       const DataMap& responsibilities) = 0;
  ...
  struct ChordPeer {
    ...
    inline void invalidate() { id = PeerId(); }
  };
  ...
  map_api_common::ReaderWriterMutex peer_lock_;
  ...
  DataMap data_;
  map_api_common::ReaderWriterMutex data_lock_;
