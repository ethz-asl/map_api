...
bool ChordIndex::handleGetClosestPrecedingFinger(const Key& key,
                                                 PeerId* result) {
...
bool ChordIndex::handleAddData(const std::string& key,
...
bool ChordIndex::handleRetrieveData(const std::string& key,
                                    std::string* value) {
  CHECK_NOTNULL(value);
  if (!waitUntilInitialized()) {
    // TODO(tcies) re-introduce request_status
...
bool ChordIndex::handleFetchResponsibilities(const PeerId& requester,
...
PeerId ChordIndex::findSuccessor(const Key& key) {
  while (true) {
    ...
    if (isIn(key, own_key_, successor_->key)) {
      ...
      peer_lock_.releaseReadLock();
      PeerId predecessor = findPredecessor(key);
...
PeerId ChordIndex::findPredecessor(const Key& key) {
  while (true) {
    ...
    while (!isIn(key, hash(result), hash(result_successor))) {
      if (!getClosestPrecedingFingerRpc(result, key, &result)) {
        try_again = true;
        break;
      }
      if (!getSuccessorRpc(result, &result_successor)) {
...
void ChordIndex::leaveClean() {
  ...
  while (true) {
    ...
    if (predecessor == PeerId::self()) {
      ...
      peer_lock_.acquireReadLock();
      self_consistent =
          predecessor_->id == predecessor && successor_->id == successor;
      peer_lock_.releaseReadLock();
      // The following request must not be declined due to locks.
      CHECK(getPredecessorRpc(successor, &successor_predecessor));
...
void ChordIndex::registerPeer(const PeerId& peer,
...
bool ChordIndex::isIn(const Key& key, const Key& from_inclusive,
...
bool ChordIndex::addDataLocally(const std::string& key,
                                const std::string& value) {
  data_lock_.acquireWriteLock();
...
bool ChordIndex::retrieveDataLocally(const std::string& key,
                                     std::string* value) {
  CHECK_NOTNULL(value);
  data_lock_.acquireReadLock();
