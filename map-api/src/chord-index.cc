...
bool ChordIndex::handleLock(const PeerId& requester) {
  if (!waitUntilInitialized()) {
    VLOG(3) << own_key_ << " has become inactive, can't be locked!";
    return false;
  }
...
bool ChordIndex::handleUnlock(const PeerId& requester) {
  ...
  if (!node_locked_ || node_lock_holder_ != requester) {
    VLOG(3) << "Unlock from " << hash(requester)
            << " rejected due to incorrectness.";
    return false;
  } else {
    node_locked_ = false;
    lock.unlock();
...
bool ChordIndex::handleReplace(const PeerId& old_peer, const PeerId& new_peer) {
  ...
  if (!successor && !predecessor) {  // could be both
    VLOG(3) << "Replace rejected due to incorrectness.";
    return false;
  }
  std::lock_guard<std::mutex> lock(node_lock_);
  if (!node_locked_ || node_lock_holder_ != old_peer) {
    peer_lock_.releaseReadLock();
    VLOG(3) << "Replace rejected because old peer is not lock holder.";
    return false;
  }
...
bool ChordIndex::handleInitReplicator(int index, const DataMap& data,
  ...
  VLOG(2) << PeerId::self() << " Replicating data of " << peer << " at index "
          << index;
  aslam::ScopedWriteLock replicated_data_lock(&replicated_data_lock_);
...
bool ChordIndex::addData(const std::string& key, const std::string& value) {
  ...
  for (size_t i = 0; i < kNumReplications; ++i) {
    PeerId replicator_i;
    if (successor == PeerId::self()) {
      aslam::ScopedReadLock peer_lock(&peer_lock_);
...
void ChordIndex::lock() {
  while (true) {
    ...
    if (!node_locked_) {
      ...
      VLOG(4) << PeerId::self() << " - " << own_key_ << " locked to self";
      node_lock_.unlock();
      break;
    } else {
      node_lock_.unlock();
...
bool ChordIndex::tryLock(const PeerId& subject) {
  ...
  return lockRpc(subject) == RpcStatus::SUCCESS;
}
...
bool ChordIndex::tryLockInOrder(PeerIdList subjects) {
  ...
  for (PeerIdList::iterator it = subjects.begin(); it != new_end; ++it) {
    VLOG(3) << own_key_ << " trying to lock " << hash(*it);
    if (!tryLock(*it)) {
      for (PeerIdList::iterator jt = subjects.begin(); jt != it; ++jt) {
        VLOG(3) << own_key_ << " unlocking " << hash(*jt);
        unlock(*jt);
      }
      return false;
    } else {
      VLOG(3) << own_key_ << " succeeded in locking " << hash(*it);
    }
  }
  return true;
}
...
void ChordIndex::unlock() {
  ...
  node_locked_ = false;
  VLOG(4) << PeerId::self() << " - " << own_key_ << " unlocked self";
}
...
/*bool ChordIndex::lockPeersInOrder(const PeerId& subject_1, 
  ...
  if (hash(subject_1) < hash(subject_2)) {
    if (!tryLock(subject_1)) {
      return false;
    }
    if (!tryLock(subject_2)) {
      unlock(subject_1);
      return false;
    }
    return true;
...
void ChordIndex::leaveClean() {
  ...
  while (true) {
    ...
    if (predecessor == PeerId::self()) {
      ...
      // The following request must not be declined due to locks.
    ...
    if (successor_id != PeerId::self()) {
      ...
      if (!successor_responding) {
        ...
        if (replace_disconnected_successor_count > 5) {
          aslam::ScopedWriteLock peer_lock(&self->peer_lock_);
        ...
        if (successor_predecessor != PeerId::self() &&
          ...
          self->peer_lock_.releaseReadLock();
          self->tryLockInOrder({successor_predecessor, successor_id});
...
bool ChordIndex::replaceDisconnectedSuccessor() {
  if (successor_failure_detected_callback_) {
    std::thread(successor_failure_detected_callback_, own_key_).detach();
  }
  ...
  if (successor_->id == predecessor_->id) {
    peer_lock_.releaseReadLock();
    aslam::ScopedWriteLock peer_lock(&peer_lock_);
    successor_ = self_;
    predecessor_ = self_;
    return true;
  }
  ...
  for (size_t i = 0; i < kNumFingers; ++i) {
    ...
    PeerId to = fingers_[i].peer->id;
    // Fix this. This is weird, they are shared pointers.
    if (i == 0 && fingers_[i].peer->id != successor_->id) {
    ...
    LOG(WARNING) << PeerId::self() << ": No response from finger " << i << ", "
                 << fingers_[i].peer->id;
  }
  peer_lock_.releaseReadLock();
  ...
  if (!candidate_predecessor.isValid()) {
    ...
    LOG(ERROR) << PeerId::self() << ": None of the fingers responded.";
    return false;
  }
  if (finger_index == 0) {
    // Existing successor active again. Nothing to do.
    VLOG(1) << PeerId::self() << ": Existing successor " << candidate
            << " responded";
    return true;
  }
  if (!tryLock(candidate)) {
    LOG(ERROR) << PeerId::self() << ": Failed to lock candidate.";
    return false;
  }
  ...
  if (candidate != successor_->id) {
    ...
    if (!fetchResponsibilitiesRpc(candidate, &data_)) {
      ...
      data_lock_.releaseWriteLock();
      LOG(ERROR) << PeerId::self()
                 << ": Failed fetchResponsibilitiesRpc to candidate.";
      return false;
    }
    data_lock_.releaseWriteLock();
    ...
    bool result = notifyRpc(candidate, PeerId::self(),
                            proto::NotifySenderType::PREDECESSOR);
    unlock(candidate);
    VLOG(1) << PeerId::self() << " successor changed to " << candidate
            << " after replaceDisconnectedSuccessor";
    if (!result) {
      LOG(ERROR) << PeerId::self() << ": Failed notify to replaced successor.";
    }
    return result;
  }
  peer_lock_.releaseReadLock();
    ...
    if (i == 0u) {
      ...
      if (!getSuccessorRpc(replicator_i_predecessor, &new_replicator_i)) {
        LOG(WARNING) << PeerId::self()
                     << " Failed fixReplicators because replicator " << i - 1
                     << " " << replicator_i_predecessor << " is offline";
        std::lock_guard<std::mutex> lock(replicator_peer_mutex_);
...
void ChordIndex::attemptDataRecovery(const Key& from) {
  CHECK(FLAGS_enable_replication);
  if (data_recovery_attempted_callback_) {
    std::thread(data_recovery_attempted_callback_, own_key_).detach();
  }
  ...
  {
    aslam::ScopedWriteLock data_lock(&data_lock_);
...
void ChordIndex::setFingerPeer(const PeerId& peer, size_t finger_index) {
  ...
  if (peer == PeerId::self()) {
    ...
    VLOG(4) << PeerId::self() << " - " << own_key_ << ": setting finger "
            << finger_index << " (" << fingers_[finger_index].base_key
            << ") to " << peer << "(" << hash(peer) << ").";
  }
}
