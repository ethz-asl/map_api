...
bool Transaction::fetchAllChunksTrackedByItemsInTable(NetTable* const table) {
  CHECK_NOTNULL(table);
  std::vector<map_api_common::Id> item_ids;
  enableDirectAccess();
  getAvailableIds(table, &item_ids);

  bool success = true;
  for (const map_api_common::Id& item_id : item_ids) {
...
void Transaction::pushNewChunkIdsToTrackers() {
  ...
  typedef std::unordered_map<map_api_common::Id, TrackeeMultimap> ItemToTrackeeMap;
