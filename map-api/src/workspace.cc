...
bool Workspace::contains(NetTable* table, const map_api_common::Id& id) const {
  CHECK_NOTNULL(table);
  // Fast default case
...
bool Workspace::TableInterface::contains(const map_api_common::Id& chunk_id) const {
...
void Workspace::TableInterface::forEachChunk(
  ...
  if (!white_found || white_submap->second.empty()) {
    ...
    for (const map_api_common::Id& chunk_id : white_submap->second) {
