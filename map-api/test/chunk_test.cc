...
TEST_F(ChunkTest, RemoteInsert) {
  ...
  if (getSubprocessId() == A) {
    IPC::barrier(INIT, 1);
    IPC::barrier(A_JOINED, 1);
    map_api_common::Id chunk_id = IPC::pop<map_api_common::Id>();
...
TEST_F(ChunkTest, Leave) {
  ...
  map_api_common::generateIdFromInt(1, &chunk_id_);
  if (getSubprocessId() == ROOT) {
    launchSubprocess(A);
...
TEST_F(ChunkTest, ChunkTransactions) {
  ...
  if (getSubprocessId() == 0) {
    ...
    ChunkBase* chunk = table_->newChunk();
    ASSERT_TRUE(chunk);
    map_api_common::Id insert_id = insert(1, chunk);
    IPC::barrier(INIT, kProcesses - 1);
    ...
    std::unordered_map<map_api_common::Id, std::shared_ptr<const Revision> >::iterator
    ...
    IPC::barrier(IDS_SHARED, kProcesses - 1);
    map_api_common::Id chunk_id = IPC::pop<map_api_common::Id>();
    map_api_common::Id item_id = IPC::pop<map_api_common::Id>();
    ChunkBase* chunk = table_->getChunk(chunk_id);
    ASSERT_TRUE(chunk);
    while (true) {
...
TEST_F(ChunkTest, SendHistory) {
  ...
  if (getSubprocessId() == ROOT) {
    ...
    item_id_ = IPC::pop<map_api_common::Id>();
    chunk_ = table_->getChunk(chunk_id_);
    IPC::barrier(DIE, 1);
