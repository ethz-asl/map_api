...
TEST_F(NetTableTest, NetTableTransactions) {
  ...
  int kCycles = 10;
  map_api_common::Id ab_chunk_id, b_chunk_id, ab_id, b_id;
  ...
  if (getSubprocessId() == A) {
    IPC::barrier(INIT, 2);
    IPC::barrier(SYNC, 2);
    ab_chunk_id = IPC::pop<map_api_common::Id>();
    b_chunk_id = IPC::pop<map_api_common::Id>();
    ab_id = IPC::pop<map_api_common::Id>();
  ...
  if (getSubprocessId() == B) {
    ...
    b_chunk_id = IPC::pop<map_api_common::Id>();
    ab_id = IPC::pop<map_api_common::Id>();
    b_id = IPC::pop<map_api_common::Id>();
    ab_chunk = table_->getChunk(ab_chunk_id);
...
TEST_F(NetTableTest, ChunkLookup) {
  ...
  if (getSubprocessId() == MASTER) {
    ...
    chunk_id = IPC::pop<map_api_common::Id>();
...
TEST_F(NetTableTestLeaveOnceShared, LeaveOnceSharedRequesting) {
  ...
  if (getSubprocessId() == MASTER) {
    ...
    chunk_id_ = IPC::pop<map_api_common::Id>();
...
class NetTableTestChunkTracking : public NetTableTest {
 protected:
  ...
  static map_api_common::Id get_tracker(const Revision& item) {
  ...
  map_api_common::Id master_chunk_id_, master_item_id_;
};
...
TEST_F(NetTableTest, GetAllIdsNoNewChunkRaceConditionThreads) {
  ...
  std::vector<map_api_common::Id> all_ids;
  do {
    Transaction transaction;
    transaction.getAvailableIds(table_, &all_ids);
    for (const map_api_common::Id& id : all_ids) {
